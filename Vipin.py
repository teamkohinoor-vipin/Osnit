...
# --- ⚙️ CONFIGURATION ---
BOT_TOKEN = "7689435166:AAHPBGaFOkcRJFDUjLCSh_KaY5TrxOisqqg"
...
UNLIMITED_GROUP_ID = -1003229958982  # Your group ID
GROUP_INVITE_LINK = "https://t.me/+iywmzmZQ7BVkZTll"  # Your group link
...
INITIAL_CREDITS = 3
REFERRAL_CREDITS = 5
SEARCH_COST = 2  # Changed from 1 to 2
REDEEM_COOLDOWN_SECONDS = 3600
REFERRAL_PREMIUM_DAYS = 1
REFERRAL_TIER_1_COUNT = 2  # We'll keep but not use for premium
REFERRAL_TIER_2_COUNT = 50  # Changed from 30 to 50
...
async def deduct_credits(user_id: int, cost: int = SEARCH_COST, chat_id: int = None) -> bool:
    # Group mein unlimited search
...
def get_info_footer(user_id: int, chat_id: int = None) -> str:
    # Group mein footer show nahi karenge
    if chat_id == UNLIMITED_GROUP_ID:
        return "\n\n🎉 <b>Group Unlimited Search Active!</b>"
    ...
    if user_id in ADMIN_IDS:
        ...
        premium_status = "♾️ <b>Unlimited Credits (50+ Referrals)</b>"
    else:
...
async def notify_referral_success(context: CallbackContext, referrer_id: int, new_user_name: str, referral_count: int):
    try:
        ...
        # Removed the 2 referrals premium bonus
...
async def notify_admin_group(context: CallbackContext, referrer_name: str, new_user_name: str, referral_count: int):
    try:
        ...
        # Removed the 2 referrals premium bonus notification
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    # Group messages ko handle karna - FIXED: Start command now works in official group
    if update.effective_chat.type != "private":
        if update.effective_chat.id == UNLIMITED_GROUP_ID:
            await update.message.reply_text(
                "🤖 <b>OSINT Bot - Group Mode</b>\n\n"
                "🔍 <b>Available Commands:</b>\n"
                "/phone <number> - Indian phone lookup\n"
                "/pakphone <number> - Pakistani phone lookup\n" 
                "/aadhaar <number> - Aadhaar lookup\n"
                "/family <number> - Family info\n"
                "/vehicle <number> - Vehicle lookup\n"
                "/ifsc <code> - IFSC lookup\n"
                "/ip <address> - IP lookup\n\n"
                "🎉 <b>Group members get UNLIMITED searches!</b>",
                parse_mode=ParseMode.HTML
            )
        return
    ...
    base_caption = (
        ...
        "💰 <b>Credit System:</b> You start with free credits. Each search costs two credits.\n\n"
        "🔗 <b>Referrals:</b> Share your link to earn more credits!\n\n"
    ...
    if user_id_str not in user_data:
        referrer_id = None
        extra_credits = 0  # Track bonus credits for new user

        if context.args and context.args[0].isdigit():
            potential_referrer_id = int(context.args[0])
            # Referrer ko check karein - agar user_data mein hai ya admin hai
            if (str(potential_referrer_id) in user_data or potential_referrer_id in ADMIN_IDS) and potential_referrer_id != user.id:
                referrer_id = potential_referrer_id

                # Add credits to referrer
                add_referral_credit(referrer_id, REFERRAL_CREDITS)

                # Increment referral count and check for rewards
                new_referral_count = increment_referral_count(referrer_id)

                # Removed: Give premium access if reached tier 1 (2 referrals)
                # Only give unlimited credits at 50 referrals
                ...
                # Give bonus credits to new user for using referral - FIXED: Now properly adds credits
                extra_credits = REFERRAL_CREDITS

                # Notify new user about referral
                try:
                    await update.message.reply_text(
                        f"🎉 You joined using a referral link! Your referrer has been rewarded with {REFERRAL_CREDITS} credits and you received {REFERRAL_CREDITS} bonus credits!",
                        parse_mode=ParseMode.HTML
                    )
                except Exception as e:
        ...
        user_data[user_id_str] = {
            "credits": INITIAL_CREDITS + extra_credits,  # Initial + bonus credits - FIXED: Now properly adds
            "referred_by": referrer_id, 
            "redeemed_codes": [], 
            "last_redeem_timestamp": 0,
            "referral_count": 0
        }
        total_credits = INITIAL_CREDITS + extra_credits
        final_caption = (f"<b>🎉 Welcome, {user.first_name}!</b>\n\n"
                         f"You have <b>{total_credits} free credits</b> to get started.\n\n{base_caption}")
        save_data(user_data, USER_DATA_FILE)
        log_user_action(user.id, "Joined", f"Referred by: {referrer_id}, Credits: {total_credits}")
    else:
...
# --- Handle when bot is added to any group ---
async def handle_bot_added_to_group(update: Update, context: CallbackContext) -> None:
    """Handle when bot is added to any group"""
    if update.message.new_chat_members:
        for member in update.message.new_chat_members:
            if member.id == context.bot.id:
                ...
                if update.effective_chat.id == UNLIMITED_GROUP_ID:
                    # Official group - send welcome message
                    await update.message.reply_text(
                        "🤖 <b>OSINT Bot - Group Mode</b>\n\n"
                        "🔍 <b>Available Commands:</b>\n"
                        "/phone <number> - Indian phone lookup\n"
                    ...
                    # Other groups - send restricted message but DON'T leave
                    await update.message.reply_text(
                        "❌ <b>I only work in the official group!</b>\n\n"
                        f"Please use me in the official group:\n{GROUP_INVITE_LINK}",
                        parse_mode=ParseMode.HTML
                    )

# --- GROUP COMMANDS ---
async def group_phone_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /phone 1234567890")
        return

    phone_number = context.args[0].strip()
    await perform_phone_lookup(update, context, phone_number)

async def group_pak_phone_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /pakphone 923001234567")
        return

    phone_number = context.args[0].strip()
    await perform_pak_phone_lookup(update, context, phone_number)

async def group_aadhaar_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /aadhaar 123456789012")
        return

    aadhaar_number = context.args[0].strip()
    await perform_aadhaar_lookup(update, context, aadhaar_number)

async def group_family_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /family 123456789012")
        return

    aadhaar_number = context.args[0].strip()
    await perform_family_lookup(update, context, aadhaar_number)

async def group_vehicle_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /vehicle DL12AB1234")
        return

    rc_number = context.args[0].strip().upper()
    await perform_vehicle_lookup(update, context, rc_number)

async def group_ifsc_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /ifsc SBIN0001234")
        return

    ifsc_code = context.args[0].strip().upper()
    await perform_ifsc_lookup(update, context, ifsc_code)

async def group_ip_lookup(update: Update, context: CallbackContext):
    if update.effective_chat.id != UNLIMITED_GROUP_ID:
        return

    if not context.args:
        await update.message.reply_text("Usage: /ip 192.168.1.1")
        return

    ip_address = context.args[0].strip()
    await perform_ip_lookup(update, context, ip_address)
...
async def button_handler(update: Update, context: CallbackContext) -> None:
    ...
    if query.data in actions:
        ...
        if referral_count > 0:
            ...
            # Removed the 2 referrals premium message
            if referral_count < REFERRAL_TIER_2_COUNT:
        ...
        keyboard = [[InlineKeyboardButton("Contact Admin 👨‍💻", url="https://t.me/KHRsupportBot")]]
...
async def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    # Group messages handle karein
    if update.effective_chat.type != "private":
        if update.effective_chat.id == UNLIMITED_GROUP_ID:
            # In official group, don't show any message for normal text
            return
        return
    ...
    if state and 'awaiting' in state and state not in ['awaiting_redeem_code']:
        if not await deduct_credits(user.id, SEARCH_COST, update.effective_chat.id):
    ...
    lookup_map = {
        'awaiting_phone': lambda: perform_phone_lookup(update, context),
        'awaiting_pak_phone': lambda: perform_pak_phone_lookup(update, context),
        'awaiting_aadhaar': lambda: perform_aadhaar_lookup(update, context),
        'awaiting_family': lambda: perform_family_lookup(update, context),
        'awaiting_vehicle': lambda: perform_vehicle_lookup(update, context),
        'awaiting_ifsc': lambda: perform_ifsc_lookup(update, context),
        'awaiting_ip': lambda: perform_ip_lookup(update, context),
    }

    if state in lookup_map:
        await lookup_map[state]()
...
# --- FAST LOOKUP FUNCTIONS (Updated for group support) ---
async def perform_phone_lookup(update: Update, context: CallbackContext, phone_number: str = None):
    if not phone_number:
    ...
    try:
        ...
        if data and isinstance(data, dict) and 'data' in data:
            ...
            await sent_message.edit_text("🤷 No details found for this number. No credits were deducted." + get_info_footer(update.effective_user.id, update.effective_chat.id))
...
async def perform_pak_phone_lookup(update: Update, context: CallbackContext, phone_number: str = None):
    if not phone_number:
        phone_number = update.message.text.strip()
    ...
    try:
        ...
        if data and isinstance(data, dict):
            ...
            address = cleaned�_data.get('address', 'N/A')
            ...
            await sent_message.edit_text("🤷 No details found for this Pakistani number. No credits were deducted." + get_info_footer(update.effective_user.id, update.effective_chat.id))
    except Exception as e:
...
async def perform_aadhaar_lookup(update: Update, context: CallbackContext, aadhaar_number: str = None):
...
async def perform_family_lookup(update: Update, context: CallbackContext, aadhaar_number: str = None):
    if not aadhaar_number:
        aadhaar_number = update.message.text.strip()
...
async def perform_vehicle_lookup(update: Update, context: CallbackContext, rc_number: str = None):
    if not rc_number:
    ...
    except requests.exceptions.HT�TPError:
        await sent_message.edit_text(f"🤷 No details found for <code>{html.escape(rc_number)}</code>." + get_info_footer(update.effective_user.id, update.effective_chat.id), parse_mode=ParseMode.HTML)
    except Exception as e:
...
async def perform_ifsc_lookup(update: Update, context: CallbackContext, ifsc_code: str = None):
    if not ifsc_code:
...
async def perform_ip_lookup(update: Update, context: CallbackContext, ip_address: str = None):
    if not ip_address:
        ...
        if state == 'admin_awaiting_add_credit' or state == 'admin_awaiting_remove_credit':
            ...
            if not history:
                ...
                for entry in history[:20]:
                    history_text += f"_{entry['timestamp']}{entry['action']}]}**: {entry['details']}\n"
                if len(history) > 20:
...
def main() -> None:
    ...
    # Start command should work in both private and groups
    application.add_handler(CommandHandler("start", start))

    # Handle when bot is added to any group
    application.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, handle_bot_added_to_group))
    ...
    application.add_handler(CommandHandler("phone", group_phone_lookup))
    application.add_handler(CommandHandler("pakphone", group_pak_phone_lookup))
    application.add_handler(CommandHandler("aadhaar", group_aadhaar_lookup))
    application.add_handler(CommandHandler("family", group_family_lookup))
    application.add_handler(CommandHandler("vehicle", group_vehicle_lookup))
    application.add_handler(CommandHandler("ifsc", group_ifsc_lookup))
    application.add_handler(CommandHandler("ip", group_ip_lookup))
